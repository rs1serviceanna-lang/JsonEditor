...
function connect_to_host_metax() {
        return new Promise((resolve, reject) => {
                ...
                metax.on("close", () => {
                        trace("host metax connection closed");
                        setTimeout(() => {
                                connect_to_host_metax();
                        }, 1000)
                })
        })
}
...
function start_write_server(port) {
        ...
        /*http_server.on('secureConnection', (socket) => {
                ...
                if (cert && cert.subject) {
                        trace(`trying new secure connection to ${socket.servername} from ${cert.subject.CN}`)
                }
        });*/
