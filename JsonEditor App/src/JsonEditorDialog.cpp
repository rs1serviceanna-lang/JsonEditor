    ...
    , webSocket(new QWebSocket(QString(), QWebSocketProtocol::VersionLatest, this))
{
    ...
    // Search in common locations relative to the executable or CWD
    QStringList certPaths = {
        "metax.crt",
        "certs/metax.crt",
        "../metax_zero_webserver/certs/metax.crt",
        "../../metax_zero_webserver/certs/metax.crt"
    };
    ...
    for (const QString &path : certPaths) {
        ...
        if (certFile.open(QIODevice::ReadOnly)) {
            ...
            QSslConfiguration::setDefaultConfiguration(sslConfig);
            qDebug() << "Trusted" << path << "for SSL/ALPN";
            break;
        }
    }
    ...
    // Default open moved to setServerUrl or command line processing
